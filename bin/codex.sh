...
- do not look in the retired, doc or build directories, these contain generated
  or stale files
...
- generate a session timestap at the beginning of each session in the form
  YYYY-MM-DD HH:mm
- check if README.md or codex.md exists and if either has changes since the
  last commit
...
- minimal implementation, do not add input checking, avoid helper function
  proliferation
- src/template.h and src/template.cpp are provided as style guides and should be
  followed especially when adding new source files, classes, methods, functions,
